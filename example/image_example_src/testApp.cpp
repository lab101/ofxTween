 ...
 */
    ofBackground(41,206,190);
	//ofSetColor(0,0,0);
	//ofNoFill();

	image.loadImage("logo.png");
	image.setPosition(0, 500);
	ofPoint p;
	p.set(800, 500, 0);
	image.moveTo(p, easingquart, 3000, 100);
	
	skull.loadImage("skull.png");
	skull.setPosition(300, 600);
	
	ofPoint p2;
	p2.set(300, 40, 0);
	skull.moveTo(p2,easingbounce, 2000,1000);
	
	skull.scaleTo(1.2, easingbounce, 1000, 0);
	skull.rotateTo(360, easingquart, 30000, 100);
}
...
void testApp::update(){
	...
	if (!image.isAnimationRunning()) {
		image.scaleTo(image.getScale() < 0.5 ? 1 : 0.4, easingquart, 800, 0);
		image.rotateTo(image.getRotation() < 1 ? 360 :0, easingquart, 800, 0);

	}
	image.update();
	
	if (!skull.isAnimationRunning()) {
		ofPoint p2;
		p2.set(300,(skull.getPosition().y == 40) ? ofGetHeight() -40 : 40, 0);
		skull.moveTo(p2,easingbounce, 2000,0);
		cout << "turn around\n";
	}
	skull.update();
}
